...
int gls_help(void)
{
  ...
  warn(
     ...
     "FN       print current gls file name\n"
  ...
  warn(
     ...
     "!        run shell command, white space after ! is required\n"
...
#define W(a,b) fwrite(a, b, 1, glsundo.file)
  ...
  W(&m, 4);
  nb = (long int) &glsgd.x0 - (long int) &glsgd.gls_file_name[0];
  W(glsgd.gls_file_name, nb);
  if (m&1) {
    nb = (long int) glsgd.lev[0].x - (long int) &glsgd.lev[0].e;
    for (j = 0; j < glsgd.nlevels; ++j) {
      W(&glsgd.lev[j].e, 60);
    }
  }
  if (m&2) {
    nb = (long int) glsgd.gam[0].x - (long int) &glsgd.gam[0].li;
    for (j = 0; j < glsgd.ngammas; ++j) {
      W(&glsgd.gam[j].li, nb);
    }
  }
...
#define R(a,b) junk = fread(a, b, 1, glsundo.file)
