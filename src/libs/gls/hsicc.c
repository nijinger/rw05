...
{
  static float e[100], ek[100], data[9], etab[10][23];
  ...
  static int ne, jj, kk, npold, swapbytes, ie, z, ioffset;
  static int skip[10], netab[10];
  static char shtab[4], shold[4];
  static char fullname[200];
  int  n, junk, idx = 0, gammanum = 0, ish = 0;
  FILE *file1, *file2;
  ...
  junk = fseek(file1, 200, SEEK_SET);
  junk = fread(&tblkey, 1, 4, file1);
  ...
  strncpy(shold, "  ", 2);
  ne = 0;
  junk = fseek(file1, (z-1)*4, SEEK_SET);
  junk = fread(&tblkey, 1, 4, file1);
  ...
  while (1) {
    junk = fseek(file2, (tblkey-1)*44, SEEK_SET);
    junk = fread(&ztab, 1, 4, file2);
    junk = fread(shtab, 1, 2, file2);
    junk = fread(data, 9, 4, file2);
    if (swapbytes) {
      swap_bytes_i(&ztab);
