...
{
  float rj1, rj2;
  int   j, nc, ix, iy, reclen, ich, icubenum, junk;
  ...
  for (icubenum = 0; icubenum < 5; icubenum++) {
	...
	junk = fseek(filez[1], iy*reclen, SEEK_SET);
	junk = fread(xxgd.spec[0], reclen, 1, filez[1]);
	for (ix = 0; ix <= iy; ++ix) {
	  ich = xxgd.luch[ix] + iy;
...
int check_band(char *ans, int nc)
{
  ...
  float r1, eg1, eg2, calc, area, finc, cnts[455], x, dcnts[455], dx;
...
int fitter3d(char mode, int maxits)
{
  ...
  int   conv = 0, i7, i8, nits, i, j, k, l, m, ichxy, ichxz, ichyz, junk;
 ...
 NEXT_IT:
  ...
  if (mode == 'I') {
    ...
    if (!get_cube_data) {
      rewind(filez[1]);
      junk = fread(save_cube, 80000, 1, filez[1]);
    }
  }
  for (iy = 0; iy < xxgd.numchs; ++iy) {
  ...
  L96:
	...
	if (mode == 'B') {
	  ...
	  if (++isave >= 20000) {
	    if (get_cube_data) {
	      junk = fwrite(save_cube, 80000, 1, filez[1]);
	    } else {
	      junk = fread(save_cube, 80000, 1, filez[1]);
	    }
	    isave = 0;
