...
int main(int argc, char **argv)
{
  ...
  junk = fseek(file, (numchs + 6)*reclen, SEEK_SET);
  junk = fread(gain, 8, 6, file);
  junk = fread(&nterms, 4, 1, file);
  ...
  junk = fseek(file, (numchs + 6)*reclen + 124, SEEK_SET);
  junk = fread(esc_file_flags, 4, 10, file);
  ...
  for (iy = 0; iy < numchs; ++iy) {
    ...
    junk = fseek(file, iy*reclen, SEEK_SET);
    junk = fread(rdata, 4, numchs, file);
    if (!esc_file_flags[2]) {
      junk = fseek(file, (iy + numchs + 7)*reclen, SEEK_SET);
      junk = fwrite(rdata, 4, numchs, file);
    }
    for (ix = 0; ix <= iy; ++ix) {
      ich = luch[ix] + iy;
      data[ich] = rdata[ix];
  ...
  junk = fseek(file, numchs*reclen, SEEK_SET);
  junk = fread(proj, 4, numchs, file);
  printf("    ...done.\n");
