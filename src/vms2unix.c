...
int eff_format(void)
{
  ...
  if (file_size == 86) junk = fread(&i, 2, 1, file);
  ...
  fclose(file);
  open_new_ofile();
  junk = fwrite(buf, pointer+4, 1, ofile);
  fclose(ofile);
  return 0;
} /* eff_format */
...
int twod_format(void)
{
  ...
  for (iy = 0; iy < numchs; ++iy) {
    ...
    junk = fread(buf, reclen, 1, file);
    pointer = 0;
    for (j = 0; j < numchs; ++j) fixr4();
    junk = fseek(file, iy*reclen, SEEK_SET);
    junk = fwrite(buf, reclen, 1, file);
  }
