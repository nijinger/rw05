...
int main(int argc, char **argv)
{
  ...
  int junk = fseek(file1, (numchs + 6)*reclen + 48, SEEK_SET);
  junk = fread(&nterms, 4, 1, file1);
  ...
  junk = fseek(file1, (numchs + 6)*reclen + 124, SEEK_SET);
  junk = fread(esc_file_flags1, 4, 10, file1);
  ...
  junk = fseek(file2, (numchs + 6)*reclen + 48, SEEK_SET);
  junk = fread(&nterms, 4, 1, file2);
  if (nterms > 6 || nterms < 2) {
    fclose(file2);
    printf("*** ERROR -- File %s\n"
	   "     seems to have the wrong byte ordering.\n"
  ...
  junk = fseek(file2, (numchs + 6)*reclen + 124, SEEK_SET);
  junk = fread(esc_file_flags2, 4, 10, file2);
  ...
  for (iy = 0; iy < numchs; ++iy) {
    ...
    junk = fseek(file1, iy*reclen, SEEK_SET);
    junk = fread(rdata1, 4, numchs, file1);
    junk = fseek(file2, iy*reclen, SEEK_SET);
    junk = fread(rdata2, 4, numchs, file2);
    ...
    if (esc_file_flags1[2]) {
      junk = fseek(file1, (iy + numchs + 7)*reclen, SEEK_SET);
      junk = fread(rdata1, 4, numchs, file1);
    }
    if (esc_file_flags2[2]) {
      junk = fseek(file2, (iy + numchs + 7)*reclen, SEEK_SET);
      junk = fread(rdata2, 4, numchs, file2);
    }
    for (ix = 0; ix < numchs; ++ix) {
